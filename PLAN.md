...
- Game API:
  ...
  - `POST /api/games/{id}/actions` applies `move`, `pivot`, `about_face`, `shoot`, `skip`, or `combat_pushback` from an action request with `playerId`, `unitId`, `type`, and type-specific fields such as `direction`, `distanceMm`, `facingDeg`, `anchorKey`, `targetUnitId`, or `combatChoice`; `pushback_150` requires `distanceMm` greater than `0` and no more than `150`.
...
- While a pending combat choice exists, legal actions are limited to `combat_pushback` with one of `pushback_150`, `withdraw_25`, or `decline`; `pushback_150` must include the requested `distanceMm`.
- A combat winner may push the losing unit up to 150mm in the winner's facing direction, withdraw its own unit 25mm backward, or decline; tied combat automatically pushes both units 25mm directly away from each other. The browser sends the clicked pushback distance along the 150mm control. Pushback and withdrawal stop before impassable terrain or the battlemap edge, ignore rough terrain, and push contacted units far enough to maintain 25mm clearance.
